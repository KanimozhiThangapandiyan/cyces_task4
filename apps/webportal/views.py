...
class PersonalDetailsViewSet(viewsets.ViewSet):
    ...
    def add_user_register_details(self, request, pk=None):
        ...
        # breakpoint()
        personal_serializer = PersonalDetailsSerializer(data=request.data['personal_details'])
        educational_serializer = EducationAndCertificationsSerializer(data=request.data['education_and_certifications'])
        #certificate_serializer = CertificationSerializer(data=request.data['certificate_details'])
        work_serializer = WorkDetailsSerializer(data=request.data['work_details'])
        employee_history_serializer = EmploymentHistorySerializer(data=request.data['employment_history'])
        awards_serializer = AwardsSerializer(data=request.data['awards'])
        preference_serializer = PreferencesSerializer(data=request.data['preferences'])

        if personal_serializer.is_valid() and educational_serializer.is_valid() and work_serializer.is_valid() and employee_history_serializer.is_valid() and awards_serializer.is_valid() and preference_serializer.is_valid():
